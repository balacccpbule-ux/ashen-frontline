...
(function () {
  ...
  function genSnow(rng, W, flags) {
    ...
    placeHouse(rng, W, flags, 6);   // v5.52 住宅楼
    // 松林（密集雪松 + 少量裸岩）
    for (let i = 0; i < 70; i++) {
  ...
  function genFort(rng, W, flags) {
    ...
    placeDestructibles(rng, W, flags);
    placeFlagCover(rng, flags);
    placeHouse(rng, W, flags, 7);   // v5.52 住宅楼
    for (let i = 0; i < 10; i++) {
  ...
  // v5.52 住宅楼：有墙、可进入（门洞）、可对外射击（窗洞）、可分级破坏（上层先塌）
  function placeHouse(rng, W, flags, n) {
    for (let i = 0; i < n; i++) {
      const spot = findSpot(rng, 7, 9, 7, 9, 14, 26, flags);
      if (!spot) continue;
      addSolid(T.buildings, 'house2', spot.x, spot.z, spot.w, spot.d, M.randRange(rng, 5.5, 7),
        { destructible: true, hp: 14000, blocksLOS: true, stages: true, rubble: true });
    }
  }
  ...
  function buildSolidMesh(scene, s) {
    ...
    if (s.kind === 'barrel') {
      ...
      // v5.52 住宅楼：四面墙（门洞+窗洞）+ 楼板 + 屋顶，可进入、可对外射击、可分级破坏
      const g = new THREE.Group();
      const wallM = new THREE.MeshStandardMaterial({ color: 0x8a867e, roughness: 0.85 });
      const roofM = new THREE.MeshStandardMaterial({ color: 0x6a5a4a, roughness: 0.9 });
      const floorM = new THREE.MeshStandardMaterial({ color: 0x55514a, roughness: 0.9 });
      const doorM = new THREE.MeshStandardMaterial({ color: 0x3a352e, roughness: 0.8 });
      const glassM = new THREE.MeshStandardMaterial({ color: 0x9fc4d8, roughness: 0.2, metalness: 0.5 });
      const hw = s.w / 2, hd = s.d / 2, t = 0.35, floorH = s.h / 2;
      // 楼板
      const ground = new THREE.Mesh(new THREE.BoxGeometry(s.w, 0.2, s.d), floorM); ground.position.set(0, 0.1, 0);
      const mid = new THREE.Mesh(new THREE.BoxGeometry(s.w, 0.18, s.d), floorM); mid.position.set(0, floorH + 0.09, 0);
      const roof = new THREE.Mesh(new THREE.BoxGeometry(s.w + 0.6, 0.25, s.d + 0.6), roofM); roof.position.set(0, s.h + 0.12, 0); roof.castShadow = true;
      g.add(ground, mid, roof);
      // 四面墙（整墙薄盒）
      const walls = new THREE.Group();
      const mkWall = (wx, wz, wlen, dlen) => { const m = new THREE.Mesh(new THREE.BoxGeometry(wlen, s.h, dlen), wallM); m.position.set(wx, s.h / 2, wz); m.castShadow = true; walls.add(m); return m; };
      mkWall(0, -hd, s.w, t);   // 前墙
      mkWall(0, hd, s.w, t);    // 后墙
      mkWall(-hw, 0, t, s.d);   // 左墙
      mkWall(hw, 0, t, s.d);    // 右墙
      // 门（前墙地面层中央）
      const door = new THREE.Mesh(new THREE.BoxGeometry(1.6, 2.2, 0.1), doorM); door.position.set(0, 1.1, -hd - 0.03); walls.add(door);
      // 窗户（玻璃矩形，墙上开口视觉提示）
      const mkWin = (wx, wz, wy, wlen, dlen) => { const m = new THREE.Mesh(new THREE.BoxGeometry(wlen, 1.0, dlen), glassM); m.position.set(wx, wy, wz); walls.add(m); };
      mkWin(0, -hd - 0.03, 3.2, 1.4, 0.06);       // 前墙二层窗
      mkWin(0, hd + 0.03, 1.4, 1.4, 0.06);        // 后墙一层窗
      mkWin(0, hd + 0.03, 3.2, 1.4, 0.06);        // 后墙二层窗
      mkWin(-hw - 0.03, 0, 1.4, 0.06, 1.4);       // 左墙一层窗
      mkWin(-hw - 0.03, 0, 3.2, 0.06, 1.4);       // 左墙二层窗
      mkWin(hw + 0.03, 0, 1.4, 0.06, 1.4);        // 右墙一层窗
      mkWin(hw + 0.03, 0, 3.2, 0.06, 1.4);        // 右墙二层窗
      g.add(walls);
      g.position.set(s.cx, s.baseH, s.cz);
      mesh = g;
      s.parts = { roof, mid, walls, extras: [mid] };
    } else {
  ...
  // ---- 圆形碰撞体 vs 实体足迹 ----
  // v5.52 住宅楼碰撞：四面墙薄盒 + 门洞/窗洞
  const HOUSE_T = 0.35, HOUSE_DOOR_HALF = 0.8, HOUSE_WIN_HALF = 0.7;
  function houseOpen(s, x, y, z) {
    const lx = x - s.cx, lz = z - s.cz, ly = y - s.baseH;
    const hw = s.w / 2, hd = s.d / 2, t = HOUSE_T + 0.05;
    if (Math.abs(lz + hd) <= t && Math.abs(lx) <= HOUSE_DOOR_HALF && ly <= 2.2) return true;  // 门洞
    const win = (ly >= 0.9 && ly <= 1.9) || (ly >= 2.7 && ly <= 3.7);
    if (!win) return false;
    if ((Math.abs(lx + hw) <= t || Math.abs(lx - hw) <= t) && Math.abs(lz) <= HOUSE_WIN_HALF) return true;  // 左/右窗
    if (Math.abs(lz - hd) <= t && Math.abs(lx) <= HOUSE_WIN_HALF) return true;  // 后墙窗
    return false;
  }
  function houseWallHit(o, d, s, maxDist) {
    const hw = s.w / 2, hd = s.d / 2, t = HOUSE_T;
    const y0 = s.baseH, y1 = s.baseH + s.h;
    const walls = [
      { min: { x: s.cx - hw, y: y0, z: s.cz - hd - t / 2 }, max: { x: s.cx + hw, y: y1, z: s.cz - hd + t / 2 } },
      { min: { x: s.cx - hw, y: y0, z: s.cz + hd - t / 2 }, max: { x: s.cx + hw, y: y1, z: s.cz + hd + t / 2 } },
      { min: { x: s.cx - hw - t / 2, y: y0, z: s.cz - hd }, max: { x: s.cx - hw + t / 2, y: y1, z: s.cz + hd } },
      { min: { x: s.cx + hw - t / 2, y: y0, z: s.cz - hd }, max: { x: s.cx + hw + t / 2, y: y1, z: s.cz + hd } },
    ];
    let best = null, bestT = maxDist;
    for (const wb of walls) {
      const tt = Game.ray.rayAABB(o, d, wb.min, wb.max);
      if (tt !== null && tt >= 0 && tt < bestT) {
        const px = o.x + d.x * tt, py = o.y + d.y * tt, pz = o.z + d.z * tt;
        if (!houseOpen(s, px, py, pz)) { bestT = tt; best = { t: tt, point: { x: px, y: py, z: pz } }; }
      }
    }
    return best;
  }
  function pushCircleBox(pos, r, bx, bz, hx, hz) {
    const cx = pos.x - bx, cz = pos.z - bz;
    ...
    if (d2 < r * r) {
      if (d2 > 1e-9) {
        ...
        if (m === left) pos.x = bx - hx - r;
        else if (m === right) pos.x = bx + hx + r;
        else if (m === top) pos.z = bz - hz - r;
        else pos.z = bz + hz + r;
      }
    }
  }
  function resolveCircle(pos, r) {
    for (const s of T.solids) {
      ...
      if (s.kind === 'house2') {
        const hw = s.w / 2, hd = s.d / 2, t = HOUSE_T;
        const boxes = [
          { bx: s.cx, bz: s.cz - hd, hx: hw, hz: t / 2, door: true },
          { bx: s.cx, bz: s.cz + hd, hx: hw, hz: t / 2 },
          { bx: s.cx - hw, bz: s.cz, hx: t / 2, hz: hd },
          { bx: s.cx + hw, bz: s.cz, hx: t / 2, hz: hd },
        ];
        for (const wb of boxes) {
          if (wb.door && Math.abs(pos.x - s.cx) < HOUSE_DOOR_HALF) continue;  // 门洞
          pushCircleBox(pos, r, wb.bx, wb.bz, wb.hx, wb.hz);
        }
        continue;
      }
      pushCircleBox(pos, r, s.cx, s.cz, s.w / 2, s.d / 2);
    }
  }
  ...
  function blocksLOS(ax, ay, az, bx, by, bz) {
    ...
    for (const s of T.solids) {
      ...
      if (s.kind === 'house2') {
        const h = houseWallHit(o, d, s, len);
        if (h && h.t >= 0.1 && h.t <= len - 0.1) return true;
        continue;
      }
  ...
  function raySolid(o, d, maxDist) {
    ...
    for (const s of T.solids) {
      if (!s.solid) continue;
      if (s.kind === 'house2') {
        const h = houseWallHit(o, d, s, bestT);
        if (h && h.t < bestT) { bestT = h.t; best = s; }
        continue;
      }
      const t = Game.ray.rayAABB(o, d, s.min, s.max);
  ...
  function setStage(s, st) {
    ...
    if (st === 1) {
      ...
      if (s.kind === 'house2') {
        // v5.52 住宅楼残破：上层（屋顶+二层楼板）塌落，墙体折成矮墙
        if (s.parts) {
          if (s.parts.roof) s.parts.roof.visible = false;
          if (s.parts.extras) for (const ex of s.parts.extras) ex.visible = false;
          if (s.parts.walls) { s.parts.walls.scale.y = 1.2 / s.h; s.parts.walls.position.y = 0.6; }
        }
        s.h = 1.2;
        s.max.y = s.baseH + 1.2;
